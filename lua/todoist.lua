...
local function create_scratch_buffer()
    ...
    vim.keymap.set("n", "q",
        "<cmd>bdelete<cr>",
        vim.tbl_extend("force", opts, { desc = "Close Todoist buffer" }))

    vim.keymap.set("n", "r",
        function() M.open() end,
        vim.tbl_extend("force", opts, { desc = "Refresh Todoist tasks" }))
    ...
    vim.keymap.set("n", "<localleader>s",
        function() M.sync() end,
        vim.tbl_extend("force", opts, { desc = "Sync buffer → Todoist" }))
...
function M.sync()
    ...
    if not buf then
        vim.notify(
            "No Todoist buffer found. Run :TodoistOpen first.",
    ...
    if #lines == 0 then
        vim.notify("Buffer is empty — nothing to sync.", vim.log.levels.WARN,
            { title = "todoist-nvim" })
        return
    end
...
function M.setup(opts)
    ...
    vim.api.nvim_create_user_command(
        "TodoistOpen",
        function() M.open() end,
        { desc = "Open Todoist tasks in a Markdown buffer", nargs = 0 })

    vim.api.nvim_create_user_command(
        "TodoistSync",
        function() M.sync() end,
        { desc = "Sync Todoist buffer changes to Todoist", nargs = 0 })
end
