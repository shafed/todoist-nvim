...
local function setup_active_keymaps(buf)
    ...
    vim.keymap.set("n", "r",              function() M.open() end,     vim.tbl_extend("force", o, { desc = "Refresh" }))
        ...
        on_exit = function(_, code)
            if code ~= 0 then
                ...
                vim.schedule(function()
                    vim.notify(msg ~= "" and msg or ("exit " .. code),
                        vim.log.levels.ERROR, { title = "todoist-nvim" })
                end)
                return
            end
...
function M.completed()
    local binary = find_binary()
    if not binary then
        vim.notify("todoist-nvim: binary not found.",
            vim.log.levels.ERROR, { title = "todoist-nvim" })
        return
    end
...
function M.restore_under_cursor(buf)
    ...
    if not task_id then
        vim.notify("No task ID found on this line.", vim.log.levels.WARN,
            { title = "todoist-nvim" })
        return
    end

    vim.notify("Restoring task " .. task_id .. "…", vim.log.levels.INFO,
        { title = "todoist-nvim" })
    ...
    vim.fn.jobstart({ binary, "reopen", task_id }, {
        ...
        on_exit = function(_, code)
            if code ~= 0 then
                local msg = table.concat(err, "\n"):gsub("%s+$", "")
                vim.schedule(function()
                    vim.notify(msg ~= "" and msg or "Restore failed.",
                        vim.log.levels.ERROR, { title = "todoist-nvim" })
                end)
                return
            end
            vim.schedule(function()
                vim.notify("Task restored!", vim.log.levels.INFO,
                    { title = "todoist-nvim" })
...
function M.sync()
    ...
    local buf = find_buf(ACTIVE_BUF_NAME)
    if not buf then
        vim.notify("No active Todoist buffer. Run :TodoistOpen first.",
    ...
    vim.fn.jobstart({ binary, "sync", tmpfile }, {
        ...
        on_exit = function(_, code)
            ...
            if code ~= 0 then
                local msg = table.concat(err, "\n"):gsub("%s+$", "")
                vim.schedule(function()
                    vim.notify(msg ~= "" and msg or "Sync failed.",
                        vim.log.levels.ERROR, { title = "todoist-nvim" })
                end)
                return
            end
...
function M.setup(opts)
    ...
    vim.api.nvim_create_user_command("TodoistOpen",
        function() M.open() end,
        { desc = "Open active Todoist tasks", nargs = 0 })

    vim.api.nvim_create_user_command("TodoistCompleted",
        function() M.completed() end,
        { desc = "Open completed Todoist tasks (last 30 days)", nargs = 0 })

    vim.api.nvim_create_user_command("TodoistSync",
        function() M.sync() end,
